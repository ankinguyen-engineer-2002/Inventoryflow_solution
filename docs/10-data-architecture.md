...
| **P3 — Supplier-confidential commercial data** | `products.dealer_cost`, `products.retail_price`; future MOQ, supply contracts | Postgres with **column-level access policy** | Dealer-admin + ops_admin; **not** exposed to marketplace_read role | per-contract; default 7 years |
| **P4 — PII** | Dealer admin user identities, account contacts | (not in product schema; lives in IAM) | Restricted | per-GDPR / per-region |

**Pricing fields (`dealer_cost`, `retail_price`) are classified P3.** The target access path: drop them from the default `GET /api/products` projection; require an explicit `?include=pricing` flag with role-based authorisation; audit every access via `ingest_audit`. Shipped vs deferred status of this control is tracked in the impl repo's `STATUS.md`.
